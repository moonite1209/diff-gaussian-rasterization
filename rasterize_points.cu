...
get_max_contributor(
	...
	torch::Tensor max_contributor = torch::full({H, W}, -1, int_opts);
	...
	if(P != 0)
	{
		...
		rendered = CudaRasterizer::Rasterizer::get_max_contributor(
		geomFunc,
		binningFunc,
		imgFunc,
		P, degree, M,
	...
	return std::make_tuple(rendered, max_contributor, max_contribute, radii, geomBuffer, binningBuffer, imgBuffer);
}